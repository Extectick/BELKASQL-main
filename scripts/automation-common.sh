...
validate_role_env() {
  case "${ROLE}" in
    ...
    observability-node)
      ...
      if [[ -z "${POSTGRES_TARGETS:-}" || -z "${NODE_EXPORTER_TARGETS:-}" || -z "${ETCD_TARGETS:-}" ]]; then
        require_vars \
...
Role summary:
  ...
  DB nodes: ${DB_NODES:-${DB_HOST_A}, ${DB_HOST_B}}
...
Role summary:
  ...
  Postgres metrics targets: ${POSTGRES_TARGETS:-${CITY_A_HOST}:${CITY_A_POSTGRES_EXPORTER_PORT}, ${CITY_B_HOST}:${CITY_B_POSTGRES_EXPORTER_PORT}}
EOF
      ;;
  esac
}
